<script>
  ...
  function toggleRightMenu() {
    rightMenuOpen.update(value => !value);
  }
</script>
  ...
  <nav class="bg-gray-300 py-4">
    <div class="flex items-center justify-between px-4">
      ...
      <div class="flex items-center">
        <button class="mr-2" on:click={toggleLeftMenu}>
          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h8m-8 6h16" />
          </svg>
        </button>
        <button on:click={toggleRightMenu}>
          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h8m-8 6h16" />
          </svg>
        </button>
...
<div class="flex h-screen w-full">
  <div class="w-0 transition-all duration-300 ease-in-out" class:menu-open={$leftMenuOpen}>
    <Sidebar position="left" onToggle={toggleLeftMenu} />
  </div>
  ...
  <div class="w-0 transition-all duration-300 ease-in-out" class:menu-open={$rightMenuOpen}>
    <Sidebar position="right" onToggle={toggleRightMenu} />
  </div>
</div>
